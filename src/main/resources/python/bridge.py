...
from typing import Any, Awaitable, Callable, Dict, List, Optional, TypeVar, cast, ParamSpec
...
class Item(ProxyBase):
    ...
    @property
    def attributes(self):
        """Attributes, such as attack damage or attack speed."""
    ...
    async def _handle_shutdown(self):
        ...
        try:
            ...
            try:
                ...
                pass
            # Close stdin to unblock the reader thread, then stop the event loop
            try:
                self._stdin.close()
            except Exception:
                pass
...
class ImageDisplay:
    ...
    def _load_pixels(image: Any) -> tuple[int, int, list[tuple[int, int, int, int]]]:
        ...
        if isinstance(image, str):
            ...
            return int(img.size[0]), int(img.size[1]), list(img.getdata())
        ...
        raise TypeError(
            "image must be a file path (str), PIL Image, or (width, height, pixel_data) tuple")
    ...
    def __init__(self, location: "Location", image: Any,
        ...
        for row in range(height):
            for col in range(width):
                ...
                if dual_sided:
                    back_yaw = yaw + 180.0
                    # Same world position as front face, just rotated 180 and nudged slightly behind
    ...
    def remove(self):
        ...
        if handles and _connection is not None:
            try:
                _connection.call_sync_raw("remove_entities", handles=handles)
...
class MeshDisplay:
    ...
    def _axes_from_normal(snx, sny, snz, ref_edge=None):
        ...
        For near-vertical normals (pitch ~±90°), *ref_edge* (dx, dy, dz) is
        used to determine yaw so the display rectangle aligns with the actual
        face edges instead of defaulting to a fixed orientation.
        """
    ...
    def _rasterize(vertices, faces, pixel_size, face_colors=None,
        ...
        # --- 1. compute per-face normals and group coplanar faces ---
        ...
        for key, group_faces in groups.items():
            ...
            # Determine if the group has a single uniform color
            uniform_argb = None
            if tex_data is None:
            ...
            # Reference edge for yaw when normal is near-vertical
            fi0 = group_faces[0]
            ...
            for side_idx, (snx, sny, snz) in enumerate(side_normals):
                ...
                # Primary face: no nudge (avoids gaps at shared edges).
                # Back face (dual-sided): small inward nudge to prevent z-fighting.
                ...
                if uniform_argb is not None:
                    # --- Single entity per face (covers bounding rect) ---
                    face_w = max_u - min_u
                    face_h = max_d - min_d
                    # TextDisplay background extends upward from entity.
                    # Place entity at face bottom, centered horizontally.
                ...
                # --- Pixel rasterization fallback (textures / vertex gradients) ---
    ...
    def update_geometry(self, vertices: list[tuple[float, float, float]],
        ...
        Reuses existing entities where possible (pool approach):
        spawns more if needed, hides excess with transparency.
        """
        if face_colors is not None:
            self._face_colors = face_colors
        ...
        # Spawn additional entities if pool is too small
        if needed > have:
        ...
        if entries and _connection is not None:
            _connection.send_fire_forget("move_entities", entries=entries)
    ...
    def remove(self) -> None:
        ...
        handles = [e._handle for e in self._entities if e._handle is not None]
        if handles and _connection is not None:
            _connection.send_fire_forget("remove_entities", handles=handles)
        self._entities.clear()
...
# NPC system
_npc_registry: Dict[str, "NPC"] = {}  # uuid -> NPC
...
class NPC:
    ...
    def __init__(self, entity: Entity, name: str = None):
        ...
        self._dialog_index: Dict[str, int] = {}  # player_uuid -> dialog step
    ...
    @classmethod
    async def spawn(cls, location: Location, entity_type: "EntityType | str" = "VILLAGER",
                    name: str = None, **kwargs: Any) -> "NPC":
    ...
    def on_right_click(self, handler: Callable):
    ...
    def dialog(self, messages: List[str], loop: bool = False):
        """Set a dialog sequence shown on right-click. Each click advances to the next message."""
        self._dialog = list(messages)
        self._dialog_loop = loop
        _ensure_npc_listener()

    async def move_to(self, location: Location, speed: float = 1.0):
        """Move the NPC to a location using pathfinding."""
        await self._entity.set_aware(True)
        result = self._entity.pathfind_to(location, speed)
        return result

    async def follow_path(self, waypoints: List[Location], loop: bool = False, speed: float = 1.0, delay: float = 0.5):
    ...
    async def _handle_interact(self, player: Player, is_right_click: bool):
